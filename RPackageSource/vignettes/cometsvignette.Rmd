...
output: 
  html_document:
    ...
    toc_float:
      collapsed: false
vignette: >
  %\VignetteIndexEntry{Running RcometsAnalytics locally}
...
Older Excel files that do not have a 
 ...
 sheet can still be used with newer versions of the RcometsAnalytics R package. The runCorr function is
 kept in the package for backwards compatibility and will return a single data frame as in the
 older versions of RcometsAnalytics. Below is a comparison of the output from a correlation analysis
 using runCorr and runModel.
```{r}
 exmodeldata <- RcometsAnalytics::getModelData(exmetabdata,modelspec="Interactive", adjvars="age_grp",
                   outcomes=c("lactose","lactate"), exposures=c("age","bmi_grp"))

 old_corr  <- RcometsAnalytics::runCorr(exmodeldata,exmetabdata,"DPP")
 new_corr  <- RcometsAnalytics::runModel(exmodeldata,exmetabdata,"DPP")
 print(old_corr)
 print(new_corr)
```
