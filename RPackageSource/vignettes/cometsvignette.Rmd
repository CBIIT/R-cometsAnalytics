...
This vignette demonstrates using the COMETS R package from the command line, while  
 the [**tutorial**](https://cbiit.github.io/R-cometsAnalytics/Tutorial/docs/) 
 demonstrates using COMETS from the GUI. 
Documentation of the COMETS R package can be found here
  [**manual**](https://github.com/CBIIT/R-cometsAnalytics/blob/gh-pages/COMETS-manual.pdf).
...
The required input file should be in excel format with the following 6 sheets: 
  ...
  5. [**Models**](https://github.com/CBIIT/R-cometsAnalytics/blob/gh-pages/Models.jpg) - models used to conduct COMETS analysis. Outcomes, exposures and adjustment can specify multiple covariates delimited by spaces (ie: age bmi).
  6. [**ModelOptions**](https://github.com/CBIIT/R-cometsAnalytics/blob/gh-pages/ModelOptions.jpg) - sheet containing model specific options

Complete documentation of the various sheets can be found in the package 
 [**documentation**](https://github.com/CBIIT/R-cometsAnalytics/blob/gh-pages/COMETS-manual.pdf).
...
### 9. Confidence intervals for effects from lm or glm.
Below is example code to compute 95% confidence intervals for the estimates from the 
glm results above in the Effects data frame. Confidence intervals for the lm results
are similar.
```{r}
...
### 10. Run Analysis on all models defined in the input Excell sheet ('super-batch' mode)
...
### 11. Backwards compatibility with older Excel files and runCorr.
Older Excel files that do not have a 
  [**ModelOptions**](https://github.com/CBIIT/R-cometsAnalytics/blob/gh-pages/ModelOptions.jpg)
 sheet can still be used with newer versions of the COMETS R package. The runCorr function is
 kept in the package for backwards compatibility and will return a single data frame as in the
 older versions of COMETS. Below is a comparison of the output from a correlation analysis
 using runCorr and runModel.
```{r}
 exmodeldata <- COMETS::getModelData(exmetabdata,modelspec="Interactive", adjvars="age_grp",
                   outcomes=c("lactose","lactate"), exposures=c("age","bmi_grp"))
