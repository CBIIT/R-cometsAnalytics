...
# Alternatively, one can filter the data by specifying a clinical variable.  For example, to only include subjects with an age > 60, one can run the following:
```{r}
...
The unstratified correlation analysis is run by calling the function *getCorr()*.  This function runs the model(s) that is(are) defined in the input data (Models tab).  
...
```{r}
  ...
  excorrdata2  <- COMETS::stratCorr(exmodeldata2,exmetabdata,"DPP")
```


###5. Run Analysis on all models defined in the input Excell sheet
